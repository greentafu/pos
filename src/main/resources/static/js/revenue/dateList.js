...
function getPage5(){
    ...
    const type = document.querySelector('input[name="classification"]:checked').value;

    $.ajax({
        url:'/api/getDateListPage',
        method:'GET',
        data: {page:page5, size:size5, startDate:startDate, endDate:endDate, posNumber:posNumber, type:Number(type)},
        success:function(data){
            ...
            contents?.forEach(content => {
                ...
                if(type==='1'){
                    const dayMap = { Sunday: '일요일', Monday: '월요일', Tuesday: '화요일', Wednesday: '수요일',
                                    Thursday: '목요일', Friday: '금요일', Saturday: '토요일'};
                    date = dayMap[date];
                }

                [date, formatComma(content.count), formatComma(price), formatComma(discount), formatComma(totalPayment),
                 formatComma(vat), formatComma(net), formatComma(cost), (profit<0)?'-'+formatComma(profit):formatComma(profit),
                 Math.round(percent)+'%'].forEach(text => {
            ...
            document.getElementById('summaryDate').textContent = startDate+'~'+endDate;
